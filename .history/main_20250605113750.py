...
class MultiDomainMLSystem:
    ...
    def intelligent_query_page(self):
        ...
        # Safe input clearing BEFORE input widget
        ...
        if question:
            ...
            try:
                    ...
                    if entity_col and entity_val and match_cols:
                        ...
                        if not filtered.empty:
                            ...
                            formatted_parts = [
                                f"{col.replace('_', ' ').capitalize()} is {val}"
                                for col, val in result.items()
                            ]
                            response_text = f"📋 The student with ID {entity_val.upper()} has the following details: " + ", and ".join(formatted_parts) + "."
                    ...
                    elif "average" in question_lower or "mean" in question_lower or "max" in question_lower or "min" in question_lower:
                        stats = []
                        for col in possible_numeric_cols:
                            if col.lower() in question_lower:
                                if "average" in question_lower or "mean" in question_lower:
                                    stats.append(f"Mean of {col}: {data[col].mean():.2f}")
                                if "max" in question_lower:
                                    stats.append(f"Max of {col}: {data[col].max():.2f}")
                                if "min" in question_lower:
                                    stats.append(f"Min of {col}: {data[col].min():.2f}")
                        if stats:
                    response_text = "\n".join(stats)
                    st.markdown(response_text)
                    st.session_state.chat_log.append({"role": "assistant", "content": response_text})
            ...
            error_text = f"❌ Error during query processing: {e}"
            st.error(error_text)
