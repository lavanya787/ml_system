    ...
    def intelligent_query_page(self):
        ...
        if question:
            ...
            if "history" in question_lower:
                ...
                handled = True
            elif re.search(r"\b(mark|score|value|grade|what about|total)\b", question_lower):
                try:
                    target_col = None
                    target_name = None
                    match_col = None
                    ...
                    if context and is_followup:
                        ...
                        all_vals = data[match_col].dropna().astype(str).unique().tolist()
                        matches = difflib.get_close_matches(question_lower, all_vals, n=1, cutoff=0.6)
                        if matches:
                        ...
                        for col in possible_text_cols:
                            all_vals = data[col].dropna().astype(str).unique().tolist()
                            matches = difflib.get_close_matches(question_lower, all_vals, n=1, cutoff=0.6)
                            if matches:
                                target_name = matches[0]
                                match_col = col
        ...
        handled = True
                ...
                try:
                    ...
                    if context and is_followup:
                        ...
                        possible_text_cols = data.select_dtypes(include='object').columns.tolist()
                        st.write("Possible ID columns:", possible_text_cols)
                        for col in possible_text_cols:
                            st.write(f"Sample values in {col}:", data[col].dropna().unique()[:5])
                        for col in possible_text_cols:
