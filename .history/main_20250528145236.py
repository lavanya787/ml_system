...
class MultiDomainMLSystem:
    ...
    def read_uploaded_file(self, uploaded_file):
        ...
        try:
            uploaded_file.seek(0)  # reset pointer to start for safe reading
            ...
            if not fullText.strip():
                st.warning("⚠️ DOCX file seems empty.")
            ...
            if not text.strip():
                st.warning("⚠️ PDF file contains no extractable text.")
        ...
        # Optionally, if you have logger:
        # self.logger.log_error(f"Failed to read {uploaded_file.name}: {e}")
        return None
...
def upload_and_detect_page(self):
    ...
    if uploaded_files:
        ...
        for uploaded_file in uploaded_files:
            ...
            detected_domain = detected.get('domain')
