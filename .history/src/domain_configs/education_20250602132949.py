...
class DomainConfig:
    def __init__(self, logger: Logger):
        ...
        self.domain_keywords = {
            'student_identifiers': [
                'student', 'pupil', 'learner', 'id', 'enrollment', 'registration', 'roll', 'admission'
            ],
            'academic_performance': [
                'grade', 'score', 'mark', 'gpa', 'result', 'achievement', 'rank', 'percentile', 'pass'
            ],
            'subjects_courses': [
                'math', 'science', 'english', 'course', 'history', 'language',
                'biology', 'physics', 'chemistry', 'literature'
            ],
            'demographics': [
                'age', 'gender', 'class', 'grade_level', 'ethnicity', 'parent', 'socioeconomic', 'disability'
            ],
            'attendance_behavior': [
                'attendance', 'absent', 'behavior', 'participation', 'tardy', 'discipline', 'engagement', 'truancy'
            ],
            'assessment': [
                'exam', 'test', 'quiz', 'assignment', 'project', 'homework', 'evaluation'
            ],
            'institution': [
                'school', 'college', 'university', 'teacher', 'faculty', 'extracurricular', 'principal', 'academy'
            ],
            'resources': [
                'syllabus', 'textbook', 'scholarship', 'curriculum', 'library', 'program'
            ]
        }
        self.llm_manager = LLMManager(logger=self.logger)
        self.logger.log_info("Initialized education DomainConfig")
    ...
    def _check_data_patterns(self, df: pd.DataFrame) -> float:
        ...
        for col in numeric_cols:
            ...
            if ('grade' in col_lower or 'score' in col_lower) and df[col].max() <= 100 and df[col].min() >= 0:
                score += 2
            elif 'age' in col_lower and 4 <= df[col].min() <= df[col].max() <= 30:
                score += 1
...
def handle_general_query(self, query: str, raw_data: pd.DataFrame, processed_data: dict) -> dict:
    ...
    def handle_prediction_query(self, query: str, raw_data: pd.DataFrame, models: Dict[str, Any]) -> Dict[str, Any]:
        ...
        model_name = max(
            models.keys(),
            key=lambda k: models[k].get('accuracy' if models[k]['model_type'] == 'classification' else 'r2', 0)
        )
        model = models[model_name]['model']

        feature_cols = [
            col for col in filtered_data.columns
            if col != target_col and filtered_data[col].dtype in ['int64', 'float64']
        ]
    ...
    def handle_general_query(self, query: str, raw_data: pd.DataFrame, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        """Handle general domain-specific queries."""
        self.logger.log_info(f"Handling general education query: {query}")
        response = self.llm_manager.generate_response(query)
        return {'summary': response, 'query_type': 'general'}
