...
class DomainConfig:
    def __init__(self, logger: Logger):
        ...
        self.domain_keywords = {
            'service': ['hotel', 'restaurant', 'booking', 'reservation', 'checkin', 'checkout'],
            'guest': ['guest', 'customer', 'visitor', 'patron', 'client'],
            'metrics': ['occupancy', 'satisfaction', 'revenue', 'rating', 'profit'],
            'facility': ['room', 'suite', 'amenity', 'facility', 'kitchen'],
            'feedback': ['review', 'feedback', 'survey', 'comment', 'rating'],
            'event': ['event', 'banquet', 'conference', 'wedding'],
            'staffing': ['employee', 'staff', 'waiter', 'chef', 'manager']
        }
    ...
    def identify_target(self, df: pd.DataFrame, detection_result: Dict[str, Any]) -> str:
        target_keywords = ['satisfaction', 'occupancy', 'revenue', 'rating']
    ...
    def engineer_features(self, df: pd.DataFrame, detection_result: Dict[str, Any]) -> pd.DataFrame:
        ...
        if 'checkin' in df.columns and 'checkout' in df.columns:
            try:
                engineered_df['stay_duration'] = (pd.to_datetime(df['checkout']) - pd.to_datetime(df['checkin'])).dt.days
                self.logger.log_info("Engineered feature: stay_duration")
            except:
                self.logger.log_warning("Could not compute stay_duration due to invalid date format")
    ...
    def handle_performance_query(self, query: str, raw_data: pd.DataFrame, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_cols = [col for col in raw_data.columns if any(k in col.lower() for k in ['satisfaction', 'revenue', 'rating'])]
        if not target_cols:
    ...
    def handle_risk_query(self, query: str, raw_data: pd.DataFrame, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        risk_cols = [col for col in raw_data.columns if any(term in col.lower() for term in ['cancellation', 'satisfaction', 'rating'])]
        if not risk_cols:
        ...
        elif 'satisfaction' in target_col.lower() or 'rating' in target_col.lower():
    ...
    def handle_general_query(self, query: str, raw_data: pd.DataFrame, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        context = f"Hospitality analysis: {len(raw_data)} records with {len(raw_data.columns)} features."
        summary = context  # Placeholder
        # summary = self.llm_manager.generate_response(context)  # Uncomment when available
