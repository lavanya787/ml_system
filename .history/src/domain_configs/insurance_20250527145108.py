...
class DomainConfig:
    def __init__(self, logger: Logger):
        ...
        self.domain_keywords = {
            'policy': ['policy', 'claim', 'id', 'insurance', 'coverage'],
            'customer': ['insured', 'client', 'policyholder', 'beneficiary'],
            'metrics': ['premium', 'claim_amount', 'risk', 'loss_ratio'],
            'underwriting': ['underwriting', 'assessment', 'evaluation'],
            'coverage': ['payout', 'deductible', 'limit', 'benefit']
        }
        self.llm_manager = LLMManager(logger=self.logger)
        self.logger.log_info("Initialized insurance DomainConfig")

    def detect_domain(self, df: pd.DataFrame) -> Dict[str, Any]:
        self.logger.log_info("Detecting insurance domain...")
        ...
        self.logger.log_info(f"Insurance domain confidence: {overall_confidence:.2f}")
        return result
    ...
    def _check_data_patterns(self, df: pd.DataFrame) -> float:
        ...
        for col in numeric_cols:
            ...
            if 'premium' in col_lower and df[col].min() >= 0:
                score += 2
            if 'claim_amount' in col_lower and df[col].min() >= 0:
                score += 1
            if 'loss_ratio' in col_lower and df[col].min() >= 0:
                score += 1
        return min(score, 10)

    def identify_target(self, df: pd.DataFrame, detection_result: Dict[str, Any]) -> str:
        target_keywords = ['claim_amount', 'risk', 'loss_ratio']
        for col in df.columns:
            if any(keyword in col.lower() for keyword in target_keywords):
                self.logger.log_info(f"Identified target variable: {col}")
                return col
        self.logger.log_warning("No target variable found for insurance domain")
        return None
    ...
    def engineer_features(self, df: pd.DataFrame, detection_result: Dict[str, Any]) -> pd.DataFrame:
        ...
        if 'premium' in df.columns and 'claims' in df.columns:
            engineered_df['premium_per_claim'] = df['premium'] / df['claims'].replace(0, np.nan)
            self.logger.log_info("Engineered feature: premium_per_claim")
        if 'claim_amount' in df.columns and 'premium' in df.columns:
            engineered_df['claim_to_premium_ratio'] = df['claim_amount'] / df['premium'].replace(0, np.nan)
            self.logger.log_info("Engineered feature: claim_to_premium_ratio")
    ...
    def handle_performance_query(self, query: str, raw_data: pd.DataFrame, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_cols = [col for col in raw_data.columns if any(k in col.lower() for k in ['premium', 'claim_amount', 'loss_ratio'])]
        if not target_cols:
    ...
    def create_analysis(self, raw_data: pd.DataFrame, processed_data: Dict[str, Any]):
        self.logger.log_info("Creating insurance-specific visualizations")
        st.write("Insurance-specific visualizations to be implemented.")