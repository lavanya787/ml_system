...
class DomainConfig:
    def __init__(self):
        self.domain_keywords = {
            'sales': ['sale', 'revenue', 'price', 'amount', 'cost'],
            'customer': ['customer', 'user', 'client', 'id'],
            'product': ['product', 'item', 'sku', 'category'],
            'transaction': ['order', 'purchase', 'transaction', 'cart'],
            'behavior': ['click', 'view', 'rating', 'review']
        }
        self.llm_manager = LLMManager()
    
    def detect_domain(self, df: pd.DataFrame) -> Dict[str, Any]:
        """Detect if dataset belongs to ecommerce domain"""
    ...
    def _check_data_patterns(self, df: pd.DataFrame) -> float:
        """Check for ecommerce data patterns"""
        score = 0
        numeric_cols = df.select_dtypes(include=[np.number]).columns
        for col in numeric_cols:
            col_lower = col.lower()
            if 'price' in col_lower or 'revenue' in col_lower:
                score += 2
        return min(score, 10)
    ...
    def engineer_features(self, df: pd.DataFrame, detection_result: Dict[str, Any]) -> pd.DataFrame:
        """Engineer ecommerce-specific features"""
    ...
    def handle_prediction_query(self, query: str, raw_data: pd.DataFrame, models: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Identify conditions (e.g., "for electronics")
        conditions = {}
