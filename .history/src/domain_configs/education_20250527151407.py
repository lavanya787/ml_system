...
class DomainConfig:
    def __init__(self, logger: Logger):
        ...
        self.domain_keywords = {
            'student_identifiers': ['student', 'pupil', 'learner', 'id', 'enrollment', 'registration', 'roll', 'admission'],
            'academic_performance': ['grade', 'score', 'mark', 'gpa', 'result', 'achievement', 'rank', 'percentile', 'pass'],
            'subjects_courses': ['math', 'science', 'english', 'course', 'history', 'language', 'biology', 'physics', 'chemistry', 'literature'],
            'demographics': ['age', 'gender', 'class', 'grade_level', 'ethnicity', 'parent', 'socioeconomic', 'disability'],
            'attendance_behavior': ['attendance', 'absent', 'behavior', 'participation', 'tardy', 'discipline', 'engagement', 'truancy'],
            'assessment': ['exam', 'test', 'quiz', 'assignment', 'project', 'homework', 'evaluation'],
            'institution': ['school', 'college', 'university', 'teacher', 'faculty', 'extracurricular', 'principal', 'academy'],
            'resources': ['syllabus', 'textbook', 'scholarship', 'curriculum', 'library', 'program']
        }
    ...
    def handle_prediction_query(self, query: str, raw_data: pd.DataFrame, models: Dict[str, Any]) -> Dict[str, Any]:
        ...
        model_name = max(models.keys(), key=lambda k: models[k].get('accuracy' if models[k]['model_type'] == 'classification' else 'r2', 0))
