...
class DomainConfig:
    def __init__(self, logger: Logger):
        ...
        self.domain_keywords = {
            'case': ['case', 'id', 'lawsuit', 'litigation', 'docket'],
            'party': ['client', 'defendant', 'plaintiff', 'attorney'],
            'metrics': ['status', 'settlement', 'duration', 'cost'],
            'documents': ['contract', 'agreement', 'filing', 'motion'],
            'court': ['court', 'hearing', 'trial', 'judge']
        }
        self.llm_manager = LLMManager(logger=self.logger)
        self.logger.log_info("Initialized legal DomainConfig")

    def detect_domain(self, df: pd.DataFrame) -> Dict[str, Any]:
        self.logger.log_info("Detecting legal domain...")
        ...
        self.logger.log_info(f"Legal domain confidence: {overall_confidence:.2f}")
        return result
    ...
    def _check_data_patterns(self, df: pd.DataFrame) -> float:
        ...
        for col in numeric_cols:
            col_lower = col.lower()
            if 'settlement' in col_lower and df[col].min() >= 0:
                score += 1
            if 'duration' in col_lower and df[col].min() >= 0:
                score += 1
            if 'cost' in col_lower and df[col].min() >= 0:
                score += 1
        return min(score, 10)

    def identify_target(self, df: pd.DataFrame, detection_result: Dict[str, Any]) -> str:
        target_keywords = ['status', 'settlement', 'duration']
        for col in df.columns:
            if any(keyword in col.lower() for keyword in target_keywords):
                self.logger.log_info(f"Identified target variable: {col}")
                return col
        self.logger.log_warning("No target variable found for legal domain")
        return None
    ...
    def engineer_features(self, df: pd.DataFrame, detection_result: Dict[str, Any]) -> pd.DataFrame:
        ...
        if 'case_date' in df.columns:
            try:
                engineered_df['case_age_days'] = (pd.to_datetime('today') - pd.to_datetime(df['case_date'])).dt.days
                self.logger.log_info("Engineered feature: case_age_days")
            except:
                self.logger.log_warning("Could not compute case_age_days due to invalid date format")
        if 'settlement' in df.columns and 'cost' in df.columns:
            engineered_df['settlement_to_cost_ratio'] = df['settlement'] / df['cost'].replace(0, np.nan)
            self.logger.log_info("Engineered feature: settlement_to_cost_ratio")
    ...
    def handle_prediction_query(self, query: str, raw_data: pd.DataFrame, models: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'civil' in query_lower:
            conditions['type'] = 'Civil'
        elif 'criminal' in query_lower:
            conditions['type'] = 'Criminal'
    ...
    def handle_performance_query(self, query: str, raw_data: pd.DataFrame, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_cols = [col for col in raw_data.columns if any(k in col.lower() for k in ['settlement', 'duration'])]
        if not target_cols:
    ...
    def handle_risk_query(self, query: str, raw_data: pd.DataFrame, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        risk_cols = [col for col in raw_data.columns if any(term in col.lower() for term in ['status', 'duration'])]
        if not risk_cols:
            self.logger.log_error("No risk-related columns found")
            return {'error': 'No risk-related columns found'}

        target_col = risk_cols[0]
        if 'status' in target_col.lower():
            at_risk = raw_data[raw_data[target_col] == 'Pending']
        elif 'duration' in target_col.lower():
    ...
    def handle_general_query(self, query: str, raw_data: pd.DataFrame, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        context = f"Legal analysis: {len(raw_data)} records with {len(raw_data.columns)} features."
        summary = context  # Placeholder
        # summary = self.llm_managerργ.log_info(f"Handling general query: {query}")
        context = f"Legal analysis: {len(raw_data)} records with {len(raw_data.columns)} features."
        summary = context  # Placeholder
        # summary = self.llm_manager.generate_response(context)  # Uncomment when available
    ...
    def create_analysis(self, raw_data: pd.DataFrame, processed_data: Dict[str, Any]):
        self.logger.log_info("Creating legal-specific visualizations")
        st.write("Legal-specific visualizations to be implemented.")