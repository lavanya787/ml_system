    ...
    def _load_domain_configs(self) -> Dict[str, Any]:
        ...
        for domain in self.domains:
            try:
                ...
                self.logger.log_info("Loaded domain config: {domain}")
            except Exception as e:
                self.logger.log_error("Failed to load domain config for '{domain}': {str(e)}")
        if not configs:
    ...
    def detect_domain(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            for domain, config in self.domain_configs.items():
                try:
                    ...
                    if 'confidence' not in result or not isinstance(result['confidence'], (float, int)):
                        self.logger.log_warning("Invalid confidence value returned by domain '{domain}'. Skipping.")
                        continue
                    results[domain] = result
                    self.logger.log_info("Domain '{domain}' confidence: {result['confidence']:.2f}")
                except Exception as e:
                    self.logger.log_error("Error during domain '{domain}' detection: {str(e)}")
            ...
            if best_result['confidence'] < 0.3:
                self.logger.log_warning("Low confidence ({best_result['confidence']:.2f}) for all domains.")
                return {
                    'domain': None,
            ...
            self.logger.log_info("Best domain match: '{best_domain}' with confidence {best_result['confidence']:.2f}")
            return {
                'domain': best_domain,
                'confidence': best_result['confidence'],
            ...
            self.logger.log_error("Domain detection failed unexpectedly: {str(e)}")
            return {'domain': None, 'confidence': 0.0, 'config': None, 'features': {}}
