...
class QueryHandler:    
    ...
    def classify_intent(self, query: str) -> tuple:
        ...
        # 🚨 Fallback: if query contains an ID-like token, force 'details'
        if re.search(r'\b[a-z]*\d{3,}\b', query_lower):
            print("DEBUG: ID pattern detected, defaulting to 'details'")
            return 'details', 0.85, detected_subjects
    ...
    def _handle_details_query(self, user_id: str, query: str, data: pd.DataFrame, detected_subjects: List[str]) -> dict:
        ...
        try:
            ...
            found_id = None
            normalized_col = data[id_column].astype(str).str.strip().str.lower()

            for pid in possible_ids:
                if pid.strip().lower() in normalized_col.values:
                    found_id = pid
                    break
            ...
            record = data[normalized_col == found_id.strip().lower()].iloc[0]
    ...
    def format_student_summary(student_id: str, row: pd.Series, subject_fields: list) -> str:
    # Capitalize fields for consistent styling
    student_id = student_id.upper()
    name_col = next((col for col in row.index if 'name' in col.lower()), None)
    name = row[name_col] if name_col else "Unknown Student"

    # Build response parts
    marks_parts = []
    for field in subject_fields:
        if "percentage" in field.lower():
            marks_parts.append(f"secured an overall percentage of {row[field]}%")
        else:
            marks_parts.append(f"scored {row[field]} marks in {field.capitalize()}")

    # Join and format the sentence
    joined = ", and ".join(marks_parts)
    return f"The student with Roll No {student_id}, named {name}, {joined}."
