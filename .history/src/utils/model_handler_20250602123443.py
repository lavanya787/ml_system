...
class ModelHandler:
    ...
    def train_model(self, df: pd.DataFrame, target_col: str, domain: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Starting training for {domain} domain with target column: {target_col}")
            ...
            if target_col not in df.columns:
                ...
                self.logger.error(f"Target column '{target_col}' not found. Available columns: {available_cols}")
                return {
            ...
            if self._domain_model_exists(domain):
                self.logger.info(f"Existing model found for {domain} domain. Retraining...")
                return self._retrain_domain_model(df, target_col, domain)
            else:
                self.logger.info(f"No existing model for {domain} domain. Training new model...")
    ...
    def _detect_target_column(self, df: pd.DataFrame, domain: str) -> str:
        ...
        for col in df.columns:
            ...
            if any(keyword in col_lower for keyword in keywords):
                self.logger.info(f"Found target column '{col}' based on keyword matching")
                return col
        ...
        if domain.lower() == 'education':
            ...
            for col in numeric_cols:
                ...
                if any(keyword in col_lower for keyword in ['total', 'percentage', 'score', 'marks']):
                    self.logger.info(f"Found education target column '{col}' from numeric columns")
                    return col
        ...
        if len(numeric_cols) > 0:
            ...
            self.logger.info(f"Using last numeric column as target: {target}")
            return target
        
        # Ultimate fallback: last column
        target = df.columns[-1]
        self.logger.info(f"Using last column as target: {target}")
        return target
    ...
    def _domain_model_exists(self, domain: str) -> bool:
        ...
        self.logger.info(f"Model exists for {domain}: {exists}")
        return exists
    ...
    def _retrain_domain_model(self, df: pd.DataFrame, target_col: str, domain: str) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"Retraining model for domain: {domain}")
            ...
            self.logger.info(f"Successfully retrained {model_name} for {domain} domain with score: {score:.4f}")
            return results
            
        except Exception as e:
            self.logger.error(f"Error retraining model for {domain}: {str(e)}", exc_info=True)
            return self._train_new_domain_model(df, target_col, domain)

    def _train_new_domain_model(self, df: pd.DataFrame, target_col: str, domain: str) -> Dict[str, Any]:
        """Train new model for domain"""
        try:
            self.logger.info(f"Training new model for domain: {domain}, target: {target_col}")
            ...
            self.logger.error(f"Error training new model: {str(e)}", exc_info=True)
            return {'error': str(e)}
    ...
    def _prepare_data_for_retraining(self, df: pd.DataFrame, target_col: str, existing_package: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.error(f"Error preparing data for retraining: {str(e)}")
    ...
    def _train_and_save_models(self, df: pd.DataFrame, target_col: str, domain: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for name, model in models.items():
                try:
                    ...
                    if is_classification:
                        ...
                        self.logger.info(f"{name} R² score: {score:.4f}, RMSE: {rmse:.4f}")
            ...
            if results:
                ...
                self.logger.info(f"Best model: {best_model_name} with score: {results[best_model_name]['score']:.4f}")
            ...
            self.logger.error(f"Error in train_and_save_models: {str(e)}", exc_info=True)
            return {'error': str(e)}
        ...
        try:
            ...
            for col in X.columns:
                if X[col].dtype == 'object':
                    self.logger.info(f"Encoding categorical feature: {col}")
                    le = LabelEncoder()
            ...
            if y.dtype == 'object':
                self.logger.info(f"Encoding categorical target: {target_col}")
                le = LabelEncoder()
            ...
            self.logger.info(f"Final data shape - X: {X.shape}, y: {y.shape}")
            return X, y
    ...
    def _is_classification(self, y) -> bool:
        ...
        self.logger.info(f"Unique target values: {unique_values}, Classification: {is_classification}")
    ...
    def _save_model_package(self, model, domain: str, model_name: str, features: List[str], 
        ...
        try:
            ...
            self.logger.error(f"Error saving model package: {str(e)}")
    ...
    def _load_model_package(self, domain: str):
        ...
        if os.path.exists(model_path):
            ...
            self.logger.info(f"Loaded model package for {domain}")
    ...
    def _save_qa_log(self):
        ...
        try:
            ...
            self.logger.error(f"Error saving Q&A log: {str(e)}")
