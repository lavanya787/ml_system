...
def detect_domain(text, domain_modules):
    scores = {}
    for name, module in domain_modules.items():
        ...
        vectorizer = TfidfVectorizer().fit([text, combined])
        vecs = vectorizer.transform([text, combined])
        score = cosine_similarity(vecs[0], vecs[1])[0][0]
        scores[name] = score
    
    best_match = max(scores.items(), key=lambda x: x[1])
    return best_match[0], best_match[1]  # domain, confidence
