    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                self.logger.log_error(f"Exception in future result: {str(e)}", exc_info=True)                    res = None
                if res:
                domain, result = res
                results[domain] = result

    if not results:
    self.logger.log_warning("No valid detection results returned by any domain configs.")        return {'domain': None, 'confidence': 0.0, 'config': None, 'features': {}}
