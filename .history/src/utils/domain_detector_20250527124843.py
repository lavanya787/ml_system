    ...
    def _load_domain_configs(self) -> Dict[str, Any]:
        ...
        for domain in self.domains:
            try:
                ...
                configs[domain] = module.DomainConfig(logger=self.logger)  # Pass logger
                self.logger.log_info(f"Loaded domain config: {domain}")
            except Exception as e:
                self.logger.log_error(f"Failed to load domain config for '{domain}': {str(e)}")  # Fixed formatting
        if not configs:
