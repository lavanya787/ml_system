...
class QueryHandler:    
    ...
    pri t(f"DEBUG: Handling flexible query for: {query}")

    try:
        id_column = self.detect_id_column(data)
        if not id_column:
            return self._error_response("Data Structure Issue", "Unable to detect ID column in dataset.")
        ...
        for col in matched_columns:
            val = record[col]
            details_lines.append(f"{col:<20}: {val}")

        content = "\n".join(details_lines)
        formatted = self._format_professional_response(f"Details for {found_id}", content, "details")
