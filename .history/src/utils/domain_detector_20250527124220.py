...
class DomainDetector:
    ...
    def detect_domain(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if not results:
                self.logger.log_warning("⚠️ No valid detection results returned by any domain configs.")
                return {'domain': None, 'confidence': 0.0, 'config': None, 'features': {}}
            ...
            if best_result['confidence'] < 0.3:
                self.logger.log_warning(f"⚠️ Low confidence ({best_result['confidence']:.2f}) for all domains.")
                return {
                    'domain': None,
