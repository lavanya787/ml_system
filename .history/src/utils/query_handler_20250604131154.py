...
class QueryHandler:    
    ...
    def handle_query(self, user_id: str, query: str, domain: str = None, data: pd.DataFrame = None, 
        ...
        """Handle incoming queries with optional model_handler support."""
        ...
        # Handle prediction with model_handler if available
            ...
            if model_handler is not None:
                try:
                    # Try to use model_handler for predictions
                    prediction_result = self._handle_prediction_with_model(query, data, model_handler)
                    return prediction_result
                except Exception as e:
                    self.logger.log_error(f"Prediction with model failed: {e}")
                    return {
                        'type': 'prediction',
            ...
            if model_handler is not None:
                try:
                    ...
                    self.logger.log_error(f"Performance analysis failed: {e}")
                    return {
                        'type': 'performance',
    ...
    def _handle_prediction_with_model(self, query: str, data: pd.DataFrame, model_handler) -> dict:
        ...
        try:
            # Check if model_handler has a predict method
            if hasattr(model_handler, 'predict'):
                # This is a basic implementation - you may need to adjust based on your model_handler's interface
    ...
    def _handle_performance_with_model(self, query: str, data: pd.DataFrame, model_handler) -> dict:
        ...
        try:
            # Check if model_handler has performance-related methods
...
if __name__ == "__main__":
    ...
    # Test query without model_handler
    ...
    # Test query with model_handler (would need actual model_handler instance)
    # result_with_model = handler.handle_query(
    #     user_id="test_user",
    #     query="predict future performance",
    #     data=sample_data,
    #     model_handler=your_model_handler_instance
    # )
