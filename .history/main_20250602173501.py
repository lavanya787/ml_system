...
class MultiDomainMLSystem:
    ...
    def _generate_user_id(self) -> str:
        ...
        # In production, this could be based on authentication
        # For demo, we'll create a persistent user ID
        if 'persistent_user_id' not in st.session_state:
            timestamp = datetime.now().isoformat()
            st.session_state['persistent_user_id'] = hashlib.md5(
                f"user_{timestamp}".encode()
            ).hexdigest()[:12]
    ...
    def run(self):
        ...
        # User ID display (for debugging/demo purposes)
        with st.sidebar:
    ...
    def intelligent_query_page(self):
        ...
        if st.session_state.get('detected_domain') is None:
            ...
            return
         # Show current session queries at the top
        ...
        st.markdown("Ask a question (e.g., 'What is the average price?', 'Show top 5 rows')")
        question = st.text_input("🔍 Enter your question",key="query_input")

        if question:
            # Add to current session queries
            query_info = {
                'query': question,
            ...
            # Check if it's a history query first
            if any(keyword in question_lower for keyword in ['history', 'past queries', 'previous questions']):
            ...
            if re.search(r"\b(top|head)\b", question_lower):
                ...
                if matched_cols:
                    ...
                    st.write(list(data.select_dtypes(include='number').columns))
            # === NEW: Handle record-specific lookups ===
            elif re.search(r"\b(mark|score|value|grade)\b.*\bof\b", question_lower):
                try:
        ...
        if target_col and target_name:
            ...
            st.warning("Could not detect the column or person from your query.")
    except Exception as e:
        st.error(f"Error processing your record-specific query: {e}")
