...
def train_model(data, file_type, original_filename):
    ...
    if X[col].dtype == 'object':
        ...
        if parsed.notnull().any():
            ...
            X[col] = LabelEncoder().fit_transform(X[col])    for col in X.columns:
            if X[col].dtype == 'object':
                try:
                    # Try parsing datetime format
                    parsed_col = pd.to_datetime(X[col], format='%d/%m/%y', errors='coerce')
                    if parsed_col.notnull().sum() > 0:
                        X[f'{col}_year'] = parsed_col.dt.year.fillna(0).astype(int)
                        X[f'{col}_month'] = parsed_col.dt.month.fillna(0).astype(int)
                        X[f'{col}_day'] = parsed_col.dt.day.fillna(0).astype(int)
                        X.drop(columns=[col], inplace=True)
                    else:
                        # If not a date, label encode
                        X[col] = X[col].fillna("Unknown")
