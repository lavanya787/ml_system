...
def train_model():
    ...
    if file and allowed_file(file.filename, ALLOWED_EXTENSIONS ):
        ...
        try:
            ...
            # Convert features to numeric, coerce errors, then drop NaNs
            X = X.apply(pd.to_numeric, errors='coerce')
            combined = pd.concat([X, y], axis=1).dropna()
            if combined.empty:
                return jsonify({"error": "All rows contain non-numeric or missing values after preprocessing"}), 400
            X = combined.iloc[:, :-1]
            y = combined.iloc[:, -1]

             if len(X) < 2:
                return jsonify({"error": "Not enough valid rows after cleaning for training"}), 400


            # Optionally, drop rows with NaN values if they were introduced
            X = X.dropna()
            y = y.loc[X.index]
